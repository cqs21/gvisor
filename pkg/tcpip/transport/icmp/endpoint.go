...
func (e *endpoint) prepareForWrite(opts tcpip.WriteOptions) (uint16, tcpip.Error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
...
func (e *endpoint) write(p tcpip.Payloader, opts tcpip.WriteOptions) (int64, tcpip.Error) {
	ident, err := e.prepareForWrite(opts)
	if err != nil {
		return 0, err
	}
	...
	if _, err := e.net.Write(func(netProto tcpip.NetworkProtocolNumber, src, dst tcpip.Address, maxHeaderLength int, requiresTXChecksum bool) (*stack.PacketBuffer, tcpip.Error) {
		...
		case header.IPv4ProtocolNumber:
			pkt, sentStat, droppedStat, err = pkt4(e.stack, ident, v, maxHeaderLength)

		case header.IPv6ProtocolNumber:
			pkt, sentStat, droppedStat, err = pkt6(e.stack, ident, v, src, dst, maxHeaderLength)
		}
		if err != nil {
			return nil, err
		}
...
func pkt4(s *stack.Stack, ident uint16, data buffer.View, maxHeaderLength int) (*stack.PacketBuffer, *tcpip.StatCounter, *tcpip.StatCounter, tcpip.Error) {
	if len(data) < header.ICMPv4MinimumSize {
		return nil, nil, nil, &tcpip.ErrInvalidEndpointState{}
	}

	pkt := stack.NewPacketBuffer(stack.PacketBufferOptions{
		ReserveHeaderBytes: header.ICMPv4MinimumSize + maxHeaderLength,
	})
...
func pkt6(s *stack.Stack, ident uint16, data buffer.View, src, dst tcpip.Address, maxHeaderLength int) (*stack.PacketBuffer, *tcpip.StatCounter, *tcpip.StatCounter, tcpip.Error) {
	if len(data) < header.ICMPv6EchoMinimumSize {
		return nil, nil, nil, &tcpip.ErrInvalidEndpointState{}
	}

	pkt := stack.NewPacketBuffer(stack.PacketBufferOptions{
		ReserveHeaderBytes: header.ICMPv6MinimumSize + maxHeaderLength,
	})
